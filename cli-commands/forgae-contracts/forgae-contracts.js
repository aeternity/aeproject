...
const configureSettings = (currentDir) => {
  ...
  fs.copyFileSync(path.join(currentDir, contractsConstants.FORGAE_SETTINGS_PATH), path.join(contractsAeppDirectory, contractsConstants.CONTRACTS_SETTINGS_PATH));
};

const updateSettingsFile = (options, currentDir) => {
  const pathToForgaeSettings = path.join(currentDir, contractsConstants.FORGAE_SETTINGS_PATH);
  const settingsContent = fs.readFileSync(pathToForgaeSettings, 'utf8');
  const jsonString = settingsContent.replace(contractsConstants.EXPORT_FILE_LITERAL, '');
